...
async def set_tilt(request: TiltMsg) -> StatusMsg:
    ...
    if 0 <= request.angle <= 15:
        _set_tilt_queue.put(request.angle)
        return StatusMsg(status="Success")
    else:
        return StatusMsg(status="Error", message="Angle must be between 0 and 15 degrees")
...
async def get_tilt() -> TiltMsg:
    ...
    return TiltMsg(angle=_get_tilt_queue.get(block=True, timeout=None))
...
def get_data() -> DataMsg:
    ...
    match data:
        case KinectDataVideo(_, data, timestamp):
            return DataMsg(type="video", data=data.tolist(), timestamp=timestamp)
        case KinectDataDepth(_, data, timestamp):
